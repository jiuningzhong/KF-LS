...
public abstract class GenesisControl {
	...
	public static class EvalCheckboxListener implements ItemListener {
		...
		public void itemStateChanged(ItemEvent ie) {
			...
			System.out.println("itemsstatechanged eva; \t" + eval);
	...
	public static class PluginCheckboxListener<E> implements ItemListener {
		...
		public void itemStateChanged(ItemEvent ie) {
			...
			System.out.println("itemsstatechanged 2 eva; \t" + check.getValue().toString() + "\t" + check.isSelected());
	...
	private static MutableTreeNode getValidationNode(Map<String, Serializable> validationSettings) {
		...
		DefaultMutableTreeNode node = new DefaultMutableTreeNode(
				"Validation: " + (test.equals("false") ? "None" : (type.equals("SUPPLY") ? testSet : "CV")));
	...
	public static MutableTreeNode getDocumentsNode(DocumentList docs) {
		...
		DefaultMutableTreeNode text = new DefaultMutableTreeNode("Text Column"
				+ (textColumns.size() == 1 ? ": " + (textColumns.iterator().next()) : "s: " + (textColumns.size())));

		if (textColumns.size() > 1)
			for (String s : textColumns) {
		...
		for (String s : filenames) {
			DefaultMutableTreeNode fileName = new DefaultMutableTreeNode(
					s.substring(s.lastIndexOf(java.io.File.separator) + 1));
			files.add(fileName);
		}
		node.add(files);
		node.add(size);
		...
		if (textColumns.size() > 1)
			node.add(
					new DefaultMutableTreeNode("Differentiate Text Fields: " + docs.getTextColumnsAreDifferentiated()));
		return node;
	}
	...
	public static MutableTreeNode getPluginNode(String label, SIDEPlugin plug, Map<String, String> keys) {
		DefaultMutableTreeNode plugin = new DefaultMutableTreeNode(
				label + (label.length() > 0 ? " " : "") + plug.toString());
	...
	public static MutableTreeNode getTableNode(FeatureTable features, String key) {
		...
		DefaultMutableTreeNode type = new DefaultMutableTreeNode(
				"Type: " + features.getClassValueType().toString().toLowerCase());
		node.add(size);
		node.add(annot);
		node.add(type);
		return node;
